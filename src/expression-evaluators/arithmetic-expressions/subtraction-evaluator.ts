...
export class SubtractionEvaluator extends ExpressionEvaluator {
    ...
    (unProcessedLhs: string, unProcessedRhs: string): { lhs: string, rhs: string } {
    ...
    private getValidSplitIndex(text: string): number {
        ...
        for (let i = 0; i < individualComponents.length - 1; i++) {
            const mergedLhs =
                individualComponents.slice(0, i + 1)
                    .join(SubtractionEvaluator.identifier);
            const mergedRhs =
                individualComponents.slice(i + 1, individualComponents.length)
                    .join(SubtractionEvaluator.identifier);
    ...
    evaluate(text: string): any {
        if (this.tryEvaluate(text)) {
            ...
            const { lhs, rhs } = SubtractionEvaluator
                .getNextAndPrevIdentifiers(unProcessedLhs, unProcessedRhs);
